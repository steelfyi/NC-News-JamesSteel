...
function findArticlesWithoutComments(articleArr, comments) {
  // Create an array of article IDs that have comments
  const articlesWithComments = comments.map((comment) => comment.article_id);

  // Filter the articleArr to find those that don't have comments
  const articlesWithoutComments = articleArr.filter((article) => {
    return !articlesWithComments.includes(article.id);
  });

  return articlesWithoutComments;
}

// Usage:
const articlesWithoutComments = findArticlesWithoutComments(
  articleArr,
  comments
);
console.log(articlesWithoutComments);
